function handles = barweb_yu(barvalues, errors, errors2, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)




%changed by liang, Apr 6, 2012
%errors are the low errors and errors are the up errors, they have the same dimension as the barvalues.
%if you want the bar is filled with color, you can change it in line 119
...
if nargin < 3
	error('Must have at least the first three arguments:  barweb(barvalues, errors, errors2, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)');
elseif nargin == 3
	width = 1;
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	handles.bars = bar(barvalues, width,'w'); hold on
	...
	for i = 1:numbars
		...
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i),errors2(:,i), 'k', 'linestyle', 'none');
	end
	set(gcf, 'color', 'white')
